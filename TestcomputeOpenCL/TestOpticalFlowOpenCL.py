...
def ImageFlowOpenCL(mat1, mat2, imagesize, kernelsize, deviceType):
    ...
    if (imagesize % blocksize != 0):
        print "The image size should be a multiple of 8"
        return [], []
    try:
    ...
    flowx, retErr1 = cl.openclqueues.enqueuereadbuffer(queue_id = queueID, 
                                                       buffer_id = bufferIDs[2], 
                                                       ByteCount = bufferSize)
    flowy, retErr2 = cl.openclqueues.enqueuereadbuffer(queue_id = queueID,
